...
public interface ReleaseDataRepository extends JpaRepository<ReleaseData, String> {
	...
	@Query(value="select sum(amount) amount, release_date releaseDate from release_data where release_date>=current_date-30 and release_date<=current_date GROUP BY release_date ORDER BY release_date asc", nativeQuery = true)
